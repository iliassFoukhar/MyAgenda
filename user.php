    ...
    <!-- NavBar -->
        ...
        <div class="row text-center">
            ...
            <div class ="col-md-3 h-50" style="border : double black 3px;border-radius : 5px;background-color:#343a40; padding : 0 0 10px 0;position :relative; right : 50px; height : 100%;">
                ...
                <h3 style="color : white;text-shadow : 1px 1px 2px #343a40;font-weight : 100;background-color : black;padding : 20px 0;">Votre compte</h3>
                <?php 
                    echo "<img class='aboutIcons' src='img/userAgenda.png'>"."<br>";
                    echo "<h5 style='color:orange;text-shadow : 1px 1px 2px black;'>Pseudo : </h5>"."<h4 class='infoText'>".$_SESSION["pseudo"]."</h4><br>";
                    echo "<h5 style='color : orange;text-shadow : 1px 1px 2px black;'>Agenda N°:</h5>"."<h4 class='infoText'>".$_SESSION["agenda"]."</h4><br>";
                ?>
                
                <a href="logout.php"><button type="button" class="btn btn-warning btn-lg">Déconnexion</button></a>
                <?php 
                echo "<p></p>";
                ?>
                 <!-- Demande d'amis -->
        <div class="text-center" style="margin-top : 30px; margin-bottom : 30px;">
            ...
            <div class ="col-md-6" style="border : double black 3px;border-radius : 5px;background-color:#343a40; padding : 0 0 10px 0;">
                ...
                <?php 
                    ...
                    $_SESSION["agendaToPost"] = $agenda;
                    echo '
                    <form method ="post" action = "sendMsg.php?id='.$agenda.'">
                    <div class="form-group shadow-textarea">
                    <label style ="color : black; text-shadow : 1px 1px 1px grey; border : solid 2px black;background-color : #77b3d4; border-radius : 20px; padding : 5px 10px;"for="pub">Publier un message sur l\'agenda de : '.'<span style="color : white;font-weight: 300;text-shadow : 1px 1px 2px black;font-size : 19px;">'.$pseudo.'</span>'.'</label>
                    <textarea class="form-control z-depth-1" name="message" rows="3" placeholder="Message..."></textarea>
                    
                    <a href="sendMsg.php?id='.$agenda.'"><button type="submit" name='."add".' class="float-right btn btn-success btn-sm" style="position :relative; right : 10px; top : 4px;">Publier</button></a></div>
                    </form> <br>';
                
                    //show mesages
                    $user = $_SESSION["agenda"];
                    $msgExist = false;
                    $sqlMsg = "SELECT * FROM message WHERE agendaId = '$user' ORDER BY date DESC;";
                ...
                echo'<form method="post" action="traitSearch.php"><div class="input-group xs-form form-xs form-1 pl-0" style=""><div class="input-group-prepend"><span class="input-group-text cyan lighten-2" style=" background-color : orange; border : 1px solid orange;"><i class="fas fa-search text-blue"aria-hidden="true"></i></span></div><input name="searchBar" class="form-control my-0 py-1" type="text" placeholder="Retrouvez un ami" aria-label="Search"></div></form><br>';
                    ...
                    while($var1 = mysqli_fetch_array($result1))
                    {
                        ...
                        while($var3 = mysqli_fetch_array($resultat3))
                        {
                            ...
                            echo '<form method="post" action="removeFriend.php"><a href="removeFriend.php"><button type="submit" name='."remove".$agendaToRemove.' class="float-right btn btn-alert btn-sm" style="position :relative; right : 10px;background-color : orange;"onMouseOver=\"this.style.color=\'orange\'\"onMouseOut=\"this.style.color=\'white\'\">x</button></a></form>';
